...
class Rhino {
     ...
     */
    constructor(accessKey, contextPath, sensitivity = 0.5, endpointDurationSec = 1.0, requireEndpoint = true, manualModelPath, manualLibraryPath) {
        if (accessKey === null ||
            accessKey === undefined ||
            accessKey.length === 0) {
            throw new errors_1.RhinoInvalidArgumentError(`No AccessKey provided to Rhino`);
        }
        ...
        if (!fs.existsSync(libraryPath)) {
            throw new errors_1.RhinoInvalidArgumentError(`File not found at 'libraryPath': ${libraryPath}`);
        }
        if (!fs.existsSync(modelPath)) {
            throw new errors_1.RhinoInvalidArgumentError(`File not found at 'modelPath': ${modelPath}`);
        }
        if (!fs.existsSync(contextPath)) {
            throw new errors_1.RhinoInvalidArgumentError(`File not found at 'contextPath': ${contextPath}`);
        }
        ...
        if (endpointDurationSec < 0.5 || endpointDurationSec > 5.0 || isNaN(endpointDurationSec)) {
            throw new RangeError(`Endpoint duration should be within [0.5, 5]: ${endpointDurationSec}`);
        }
        const pvRhino = require(libraryPath);
        let rhinoHandleAndStatus = null;
        try {
            rhinoHandleAndStatus = pvRhino.init(accessKey, modelPath, contextPath, sensitivity, endpointDurationSec, requireEndpoint);
        }
        catch (err) {
        ...
        if (status !== pv_status_t_1.default.SUCCESS) {
            (0, errors_1.pvStatusToException)(status, "Rhino failed to initialize");
        }
    ...
    process(frame) {
        if (this._handle === 0 ||
            ...
            throw new errors_1.RhinoInvalidStateError("Rhino is not initialized");
        }
        if (frame === undefined || frame === null) {
            throw new errors_1.RhinoInvalidArgumentError(`Frame array provided to process() is undefined or null`);
        }
        else if (frame.length !== this.frameLength) {
            throw new errors_1.RhinoInvalidArgumentError(`Size of frame array provided to 'process' (${frame.length}) does not match the engine 'frameLength' (${this.frameLength})`);
        }
        // sample the first frame to check for non-integer values
        if (!Number.isInteger(frame[0])) {
            throw new errors_1.RhinoInvalidArgumentError(`Non-integer frame values provided to process(): ${frame[0]}. Rhino requires 16-bit integers`);
        }
        ...
        if (status !== pv_status_t_1.default.SUCCESS) {
            (0, errors_1.pvStatusToException)(status, "Rhino failed to process the frame");
        }
    ...
    getInference() {
        if (!this.isFinalized) {
            throw new errors_1.RhinoInvalidStateError("'getInference' was called but Rhino has not yet reached a conclusion. Use the results of calling process to determine if Rhino has concluded");
        }
        if (this._handle === 0 ||
            this._handle === null ||
            this._handle === undefined) {
        ...
        if (status !== pv_status_t_1.default.SUCCESS) {
            (0, errors_1.pvStatusToException)(status, `Rhino failed to get inference: ${status}`);
        }
        let inference = {
    ...
    getContextInfo() {
        ...
        if (status !== pv_status_t_1.default.SUCCESS) {
            (0, errors_1.pvStatusToException)(status, `Rhino failed to get context info: ${status}`);
        }
