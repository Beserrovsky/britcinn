...
const rhino_node_1 = require("@picovoice/rhino-node");
...
class Picovoice {
     ...
     */
    constructor(accessKey, keywordPath, wakeWordCallback, contextPath, inferenceCallback, porcupineSensitivity = 0.5, rhinoSensitivity = 0.5, endpointDurationSec = 1.0, requireEndpoint = true, porcupineModelPath, rhinoModelPath, porcupineLibraryPath, rhinoLibraryPath) {
        if (accessKey === null ||
            accessKey === undefined ||
            accessKey.length === 0) {
            throw new errors_1.PicovoiceInvalidArgumentError(`No AccessKey provided to Picovoice`);
        }
        if (!(wakeWordCallback instanceof Function)) {
            throw new errors_1.PicovoiceInvalidArgumentError("Parameter 'wakeWordCallback' is not a function");
        }
        this.wakeWordCallback = wakeWordCallback;
        if (!(inferenceCallback instanceof Function)) {
            throw new errors_1.PicovoiceInvalidArgumentError("Parameter 'inferenceCallback' is not a function");
        }
        this.inferenceCallback = inferenceCallback;
        this.porcupine = new porcupine_node_1.Porcupine(accessKey, [keywordPath], [porcupineSensitivity], porcupineModelPath, porcupineLibraryPath);
        this.rhino = new rhino_node_1.Rhino(accessKey, contextPath, rhinoSensitivity, endpointDurationSec, requireEndpoint, rhinoModelPath, rhinoLibraryPath);
    ...
    process(frame) {
        if (this.porcupine === null || this.rhino === null) {
            throw new errors_1.PicovoiceInvalidStateError("Attempting to process but resources have been released.");
        }
...
exports.default = Picovoice;
//# sourceMappingURL=picovoice.js.map